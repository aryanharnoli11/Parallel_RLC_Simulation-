...
const DECIMAL_VALUE_PATTERN = /^\d+(?:\.\d{1,2})?$/
...
const getRangeError = (field, rawValue) => {
  ...
  if (!trimmedValue || !DECIMAL_VALUE_PATTERN.test(trimmedValue) || !Number.isFinite(numericValue)) {
    return `${field.dataLabel} must be a valid number with up to 2 decimal places.`
  }
...
const handleFieldChange = (rowId, key, value) => {
  ...
  if (field) {
    ...
    // Accept whole numbers or values with no more than two decimal places.
                          ...
                          <td className={isKnown ? 'is-known-value' : 'is-entry-value'} data-label={dataLabel} key={key}>
                            <div className="calculation-input-shell">
                              <input
                                ...
                                inputMode="decimal"
                                title={validationError || (!isKnown ? `Enter a value from ${min} to ${max}, using up to 2 decimal places.` : undefined)}
                              />
