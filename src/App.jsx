...
const App = () => {
  ...
  const derivedStep = useMemo(() => {
    ...
    if (!reportGenerated) return STEPS.GENERATE_REPORT;
    if (currentStep === STEPS.RESET_SIMULATION) return STEPS.RESET_SIMULATION;
    return STEPS.PRINT_REPORT;
  }, [currentStep, connectionsVerified, powerOn, componentsSelected, variacPowered, voltage, observations.length, isResistorCorrect, reportGenerated]);
...
const handlePrint = () => {
  ...
  if (reportGenerated) {
    setCurrentStep(STEPS.RESET_SIMULATION)
    setStatus('Print dialog opened. Click Reset when you are ready to restart the simulation.')
  }
...
const handleGenerateReport = () => {
  ...
  showAlert({
    ...
    onConfirm: () => {
      ...
      setCurrentStep(STEPS.PRINT_REPORT)
      setStatus(
            ...
            <section className={`workspace-grid ${isModalOpen ? 'workspace-grid--instructions-open' : ''}`}>
              ...
              simulation accurately at each step.</p>
                        ...
                        <p className={isStepActive(STEPS.CHECK) ? 'instruction-step-active' : (isStepCompleted(STEPS.CHECK) ? 'instruction-step-completed' : '')}>
                          <strong>Step 2:</strong> Now, click on the <strong>'CHECK'</strong> button to verify the connections.
                        </p>
                        ...
                        <p className={isStepActive(STEPS.POWER_ON) ? 'instruction-step-active' : (isStepCompleted(STEPS.POWER_ON) ? 'instruction-step-completed' : '')}>
                          <strong>Step 3:</strong> Turn ON the MCB by clicking on the MCB lever.
                        </p>
                        ...
                        <p className={isStepActive(STEPS.VARIAC_ON) ? 'instruction-step-active' : (isStepCompleted(STEPS.VARIAC_ON) ? 'instruction-step-completed' : '')}>
                          <strong>Step 5:</strong> Switch ON the autotransformer by clicking on the variac power button.
                        </p>
                        ...
                        <p className={isStepActive(STEPS.SET_VOLTAGE) ? 'instruction-step-active' : (isStepCompleted(STEPS.SET_VOLTAGE) ? 'instruction-step-completed' : '')}>
                          <strong>Step 6:</strong> Click on the Variac knob to rotate it to the 30 V position.
                        </p>
                        ...
                        <p className={isStepActive(STEPS.ADD_READING) ? 'instruction-step-active' : (isStepCompleted(STEPS.ADD_READING) ? 'instruction-step-completed' : '')}>
                          <strong>Step 7:</strong> Click on the <strong>'ADD'</strong> button to record the reading. Choose another R-L-C combination and repeat the same process until atleast five readings are recorded.
                        </p>
                        ...
                        <p className={isStepActive(STEPS.CALCULATE) ? 'instruction-step-active' : (isStepCompleted(STEPS.CALCULATE) ? 'instruction-step-completed' : '')}>
                          <strong>Step 8:</strong> After five readings, select a recorded reading in the theoretical verification section, calculate the theoretical values, and click on the <strong>Verify</strong> button to verify the readings.
                        </p>
                        <p className="instructions-note"><strong>Note:</strong>After the first verification fails, the <strong>Correct Values</strong> button will be enabled for your reference.</p>

                        <p className={isStepActive(STEPS.GENERATE_REPORT) ? 'instruction-step-active' : (isStepCompleted(STEPS.GENERATE_REPORT) ? 'instruction-step-completed' : '')}>
                          <strong>Step 9:</strong> Click on the <strong>Generate Report</strong> button to generate the simulation report.
                        </p>
                        <p className={isStepActive(STEPS.PRINT_REPORT) ? 'instruction-step-active' : (isStepCompleted(STEPS.PRINT_REPORT) ? 'instruction-step-completed' : '')}>
                          <strong>Step 10:</strong> Click on the <strong>PRINT</strong> button to print the simulation or simulation report.
                        </p>
                        <p className={isStepActive(STEPS.RESET_SIMULATION) ? 'instruction-step-active' : ''}>
                          <strong>Step 11:</strong> Click on the <strong>RESET</strong> button to restart the simulation.
                        </p>
                      </div>
